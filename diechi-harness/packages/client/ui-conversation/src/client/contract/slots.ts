...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * The hero-phase quick-action row: one card-style entry per product
     * surface (skill store, training workshop, ...). Additive list so
     * several features can contribute cards beside the workspace row
     * without touching the owner; entries render by ascending order.
     * Root scope: no session exists yet, so each card opens a frame-wide
     * surface rather than acting on a current session.
     */
    'conversation.hero.quick': { kind: 'list'; scope: 'root' }
