...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    // An empty string is an absent credential, not a malformed reference: the
    // settings document (or a hand edit) may spell "no key needed" as `''`,
    // and refusing it here would fail the whole namespace's registration and
    // strand every provider under it.
    const normalizedApiKeyEnv = apiKeyEnv === undefined || apiKeyEnv === '' ? undefined : apiKeyEnv
    resolved.set(provider, {
      ...rest,
      provider,
      displayName,
      ...normalizedApiKeyEnv === undefined ? {} : { apiKeyEnv: credentialRef(normalizedApiKeyEnv) },
      streamIdleTimeoutMs,
      ...
      piProvider: buildProvider({
        ...
        namesCredential: normalizedApiKeyEnv !== undefined,
      }),
    })
  }
  return resolved
