...
const out = await page.evaluate(async () => {
  const rpc = async (method, payload) => await (await fetch('/api/' + method, {
    ...
    body: JSON.stringify({ type: 'client-request', rpcId: 'x-' + Math.random(), method, payload }),
  })).json()
  ...
  const u = await rpc('settings.update', { namespace: 'skill-store', path: 'skills', value: reset })
  ...
  return {
    ...
    sections: Object.keys(ns2 ?? {}),
    userSkills: (ns2?.user?.skills ?? []).map(s => [s.id, s.enabled]),
  }
})
